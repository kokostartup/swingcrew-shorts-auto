...
- JSON 외 다른 출력 금지
- **em-dash (`—`) / en-dash (`–`) / 콜론(`:`) 절대 사용 금지** — title/description 모두.
  영어 스타일 부호 강조 X. 한국 감성 X. 한국어 콘텐츠는 자연스러운 문장 흐름이 우선.
- 구분 필요 시: 마침표(`.`), 쉼표(`,`), 물음표(`?`), 느낌표(`!`), 또는 그냥 단어 연결.
  hyphen(`-`)도 단어 안에만 (예: "5-iron"). 문장 구분용 dash 금지.

  ❌ 나쁜 예 (영어 스타일):
  - "비거리 20m 늘리는 세 가지 — 티 높이·볼 포지션·체중이동"
  - "프로의 비결: 헤드 스피드 +5"
  - "아이언 잘 맞추는 법 — 손목 플리핑 잡기"

  ✅ 좋은 예 (한국 감성):
  - "비거리 20m 늘리는 세 가지, 티 높이·볼 포지션·체중이동"
  - "프로의 비결, 헤드 스피드 +5 만드는 법"
  - "아이언 잘 맞추는 법, 손목 플리핑 잡기"
  - "비거리 20m 늘리려면 이 세 가지만 바꾸세요"
  - "헤드 스피드 50, 비거리 250m 가능합니다"
