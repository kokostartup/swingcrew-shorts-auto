...
def conn(
    ...
    monkeypatch.setattr(
        "app.config.settings.shorts_output_dir",
        tmp_path / "out",
    )
...
def test_sync_creates_pages_and_persists_page_id(
    ...
    with patch(
        "app.pipeline.approve.notion_create_page",
        return_value="page-1",
    ) as mock_create:
    ...
    row = conn.execute(
        "SELECT notion_page_id FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_poll_syncs_approved_status_and_scheduled_at(
    ...
    def fake_list(status: str, channel: str = "ko") -> list[dict[str, object]]:
        if status == "approved":
            return [
                {
                    "id": "p1",
    ...
    row = conn.execute(
        "SELECT status, scheduled_at FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_poll_syncs_rejected(conn: sqlite3.Connection) -> None:
    ...
    row = conn.execute(
        "SELECT status FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_poll_scene_type_override_from_notion(conn: sqlite3.Connection) -> None:
    ...
    row = conn.execute(
        "SELECT scene_type FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_process_approved_runs_ffmpeg_and_updates_status(
    ...
    with (
        patch(
            "app.pipeline.approve.get_video_by_youtube_id",
            return_value=video,
        ),
        patch(
        ...
        patch(
            "app.pipeline.approve.generate_publish_meta",
    ...
    row = conn.execute(
        "SELECT status, generated_path FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_process_approved_assigns_internal_id_s01(
    ...
    row = conn.execute(
        "SELECT internal_id, generated_path FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
...
def test_process_approved_assigns_next_s_number(
    ...
    conn.execute(
        ...
        "internal_id) VALUES (?, 0, 10, 'generated', '26-B001-S01')",
        (vid,),
    )
    conn.execute(
        "INSERT INTO shorts (source_video_id, start_time, end_time, status, "
        "internal_id) VALUES (?, 60, 70, 'generated', '26-B001-S02')",
        (vid,),
    )
    sid = _insert_short(conn, vid, 100.0, 130.0)
    conn.execute(
        "UPDATE shorts SET notion_page_id = 'p1', status = 'approved' WHERE id = ?",
        (sid,),
    )
    conn.commit()
    ...
    row = conn.execute(
        "SELECT internal_id FROM shorts WHERE id = ?",
        (sid,),
    ).fetchone()
